...
function buildTables(): LabTable[] {
  const customers: DataRow[] = Array.from({ length: 600 }, (_, index) => ({
    id: index + 1,
  ...
  const products: DataRow[] = Array.from({ length: 300 }, (_, index) => ({
    id: index + 1,
  ...
  const orders: DataRow[] = Array.from({ length: 5000 }, (_, index) => ({
    id: index + 1,
  ...
  for (let index = 0; index < 12400; index += 1) {
...
ORDER BY revenue DESC;`,
  ...
  {
    id: "catalog-search",
    label: "Function-wrapped text search",
...
export function analyzeQuery(sql: string): QueryAnalysis {
  ...
  tables.forEach((name) => {
    ...
    if (name === "orders" && plainDateRange) {
      scanned = Math.round(scanned * 0.34);
      detail = "Range scan on orders.created_at";
    ...
    touchedRows += scanned;
    plan.push({ operation: "SCAN", detail, rows: scanned, cost: scanned, tone: "scan" });
  });

  let streamRows = Math.max(1, Math.min(...(plan.length ? plan.map((step) => step.rows) : [1])));
  if (hasWhere) {
  ...
  for (let index = 0; index < joinCount; index += 1) {
    ...
    plan.push({
      ...
      rows: streamRows,
      cost: Math.round(streamRows * 0.72),
      tone: "join",
    });
  }
...
function stableRows(rows: DataRow[]): string {
  const canonical = rows.map((row) => JSON.stringify(Object.fromEntries(Object.entries(row).sort(([left], [right]) => left.localeCompare(right)))));
