      ...
      <section className="workspace">
        <header className="topbar">
          ...
          <div className="top-actions">
            ...
            {showAbout && <div className="about-popover"><strong>The samples are inputs, not answer keys.</strong><p>Every plan, finding, join hotspot, rewrite, materialization, and parallelism estimate is generated from the submitted SQL, actual table statistics, and the workload recorded in this browser.</p><button type="button" onClick={() => setShowAbout(false)}>Got it</button></div>}
          </div>
        </header>
        ...
        <div className="work-grid">
          ...
          <aside className={`score-card ${healthClass}`}>
            ...
            <div className="metric-row"><span>Rows touched</span><strong>{formatNumber(analysis.touchedRows)}</strong></div><div className="metric-row"><span>Result rows</span><strong>{run ? formatNumber(run.rows.length) : "—"}</strong></div><div className="metric-row"><span>Runtime</span><strong>{run ? `${run.elapsedMs.toFixed(2)} ms` : "—"}</strong></div><div className="metric-row"><span>Parallel plan</span><strong>{analysis.parallelism.maxWorkers} workers / {analysis.parallelism.waves} waves</strong></div><div className="metric-row"><span>Work units</span><strong>{formatNumber(analysis.estimatedCost)}</strong></div>
          </aside>
        </div>
